...
<div class="container d-flex mt-5 justify-content-center">
	<div class="w-75 shadow p-5 rounded border">
		...
		<c:if test="${board.writer == user.userName && admin == null}">
			<button type="submit" id="btn-modify" class="btn btn-primary me-2">수정하기</button>
		</c:if>
		<c:if test="${admin != null}">
			<button type="submit" id="btn-modify" class="btn btn-primary me-2">수정하기</button>
		</c:if>
			<button type="button" id="btn-list" class="btn btn-primary">목록으로</button>
		</div>
...
<script src="${path}/resources/js/recruitreply.js"></script>
...
/* 		function displayList(){
		...
		addBtn.on("click", function(e){
			...
			var reply = {
		 	...
		 	if (taReplyContents.val().length > 500){
		 		alert("댓글 내용은 500 글자를 초과할 수 없습니다");
		 		taReplyContents.focus();
		 		return;
		 	}
...
//				displayList();
				// 여기서 viewPage 클릭했을 때의 페이지 번호값을 갖고 있음
				displayList(viewPage);
			});
		});
...
//				displayList();				
		...
		$("#btn-list").click(() => {
			location.href="<c:url value='/recruitrecruit.do?viewPage=${pDto.viewPage}&searchType=${pDto.searchType}&keyWord=${pDto.keyWord}'/>";
		})
		
		$("#btn-modify").click(() => {
			location.href="<c:url value='/recruitmodify.do?bid=${board.bid}&viewPage=${pDto.viewPage}&searchType=${pDto.searchType}&keyWord=${pDto.keyWord}'/>";
		})
	});
</script>
