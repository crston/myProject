 ...
 */
 ...
 var replyFunc = (function(){
 	...
 	function get(rno, cb){
 		...
 		$.get("/bbs06/evreplies/"+rno+".json", function(result){
 			if(cb) cb(result);
 		}).fail(function(){
 			alert("요청실패!!");
 		});
...
// 		$.getJSON("/bbs06/evreplies/rList/"+bid+".json",
// 			function(result){
// 				if(cb) cb(result);
...
// 	}
 	...
 	function getList(param, cb){
 		...
 		// $.ajax()이용할 경우에는 dataType을 'json' 설정
 		$.getJSON("/bbs06/evreplies/rList/"+bid+"/"+viewPage+".json",
