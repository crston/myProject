 ...
 */
 var replyFunc = (function(){
 	...
 	function get(rno, cb){
 		...
 		$.get("/bbs06/recruitreplies/"+rno+".json", function(result){
 			if(cb) cb(result);
 		}).fail(function(){
 			alert("요청실패!!");
 		});
...
// 		$.getJSON("/bbs06/recruitreplies/rList/"+bid+".json",
// 			function(result){
// 				if(cb) cb(result);
...
// 	}
 	...
 	function getList(param, cb){
 		...
 		// $.ajax()이용할 경우에는 dataType을 'json' 설정
 		$.getJSON("/bbs06/recruitreplies/rList/"+bid+"/"+viewPage+".json",
 	...
 	function update(reply, cb){
 		$.ajax({
 			type:'put',
 			url:'/bbs06/recruitreplies/'+reply.rno,
