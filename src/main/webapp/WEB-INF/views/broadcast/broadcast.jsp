...
<section class="notice">
    <div class="page-title">
      ...
      <!-- board seach area -->
	<form id="searchForm" method="post" action="bdbroadcast.do">
      ...
      <div id="board-list">
          <div class="container">
              <table class="board-table">
                  ...
                  <tr>
                      <td>${bno}</td>
		              <td><a href="<c:url value='/bdview.do?bid=${dto.bid}&viewPage=${pDto.viewPage}&searchType=${pDto.searchType}&keyWord=${pDto.keyWord}'/>"/>${dto.subject}</a></td>
			          <td>${dto.hit}</td>
			          <td>${dto.writer}</td>
...
<ul class="pagination justify-content-center my-5">
  <li class="page-item ${pDto.prevPage <= 0 ? 'disabled' : ''}">
  	<a class="page-link" href="bdbroadcast.do?viewPage=${pDto.prevPage}&cntPerPage=${pDto.cntPerPage}">이전</a>
  </li>
  
  <c:forEach var="i" begin="${pDto.blockStart}" end="${pDto.blockEnd}">
  	<li class="page-item ${pDto.viewPage == i ? 'active' : ''}">
  		<a class="page-link" href="bdbroadcast.do?viewPage=${i}&cntPerPage=${pDto.cntPerPage}&searchType=${pDto.searchType}&keyWord=${pDto.keyWord}">${i}</a>
  	</li>
  </c:forEach>
  <li class="page-item ${pDto.blockEnd >= pDto.totalPage ? 'disabled' : ''}">
  	<a class="page-link" href="bdbroadcast.do?viewPage=${pDto.nextPage}&cntPerPage=${pDto.cntPerPage}">다음</a>
  </li>
</ul>
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#btn-write").click(() => {
			location.href="<c:url value='bdregister.do'/>";
		})
		
		$("#cntPerPage").change(function(e){
			var cntVal = $("#cntPerPage option:selected").val();
			// alert(cntVal);
/* 			location.href="<c:url value='bdbroadcast.do?viewPage=${pDto.viewPage}&cntPerPage='/>"+cntVal; */	
			location.href="<c:url value='bdbroadcast.do?viewPage=1&cntPerPage='/>"+cntVal;	
		});
